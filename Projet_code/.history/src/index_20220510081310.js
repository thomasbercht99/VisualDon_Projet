...
try {
  ...
  //Insertion de la div virtuel pour le tooltip et ajoute l'opasité a 0 pour qu'il soit invisible
  ...
  d3.selectAll(".muscles")
    ...
    .on("mouseover", function (evt) {
      ...
      //Insertion du html dans le tooltip deja visible et puis ajout de ses coordonnés pour son positionnelent
    ...
    .on("click", function () {
      //Ecouteur onclick
      //Cette condition verifie d'abord s'il a des exercises qui correspond au muscle actuel et si oui il redirge vers la page de details si non il ne fait rien
  ...
  if (target) {
    //ce filtre selection tous les exercies correspondant au target passer sur l'url)
    const selectedExc = data.filter(a => a.name != null && (a.bodyPart === target || a.target === target))
    const html = []
    //Puis on boucle sur l'ensemble des exercices correspondant en construction le html qui sera inserer dans la dom. cet html es poussé dans un tableau.
